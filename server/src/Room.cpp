...
void
Room::playerHasNoCards(void * player)
{
	pthread_mutex_lock(&removePlayerMutex);
	if (isQuestioning && player == getPlayerByGuest(guestsOrder[curAnswerIndex]))
	{
		Player * pl = (Player *)player;
		if (!searchCard(suspectAp, suspectGt, suspectWp, pl->cards))
		{
			pthread_cancel(waitAnswerThread);
			Player ** pls = (Player **)players;
			char gt = guestsOrder[curAnswerIndex];
...
void
Room::playerEndsTurn(void * player)
{
	if (!isQuestioning)
	{
		Player * pl = (Player *)player;
		char gt = guestsOrder[curGuestIndex];
		if (gt > 10) gt -= 10;
		if (pl->guest == gt)
		{
			// Probably this will problem
			pthread_cancel(waitNextMoveThread);
			nextMove();
		}
	}
